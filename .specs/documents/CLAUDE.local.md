...
- **Icônes:** Lucide React (OBLIGATOIRE)
...
### 13. HeroUI Components

```typescript
// Utilisation des composants HeroUI
...
**Règle:** 
- Utiliser HeroUI pour tous les composants UI. Les composants sont disponibles via les packages @heroui/*.
- **INTERDICTION STRICTE:** Ne PAS installer d'autres bibliothèques UI (Material-UI, Ant Design, Chakra UI, Mantine, shadcn/ui, etc.) sans autorisation explicite de l'utilisateur.
- Si un composant HeroUI ne répond pas aux besoins, créer un composant personnalisé basé sur HeroUI ou demander l'autorisation avant d'installer une autre bibliothèque UI.
- Les composants HeroUI sont dans `@/components` et utilisent le système de thème HeroUI.

### 14. Lucide React - Icônes (OBLIGATOIRE)
...
// Avec Tailwind pour la taille et les couleurs
...
**Règle:** 
- TOUJOURS utiliser `lucide-react` pour tous les icônes dans le projet
- NE PAS utiliser d'autres bibliothèques d'icônes (Heroicons, React Icons, etc.)
- Importer uniquement les icônes nécessaires pour optimiser le bundle
- Utiliser les classes Tailwind pour la taille (`w-*`, `h-*`) et les couleurs
- Les icônes sont des composants React, donc utilisables directement dans JSX

**Exemples d'imports:**
```typescript
// ✅ Correct - Import nommé
import { GraduationCap, User, Settings } from 'lucide-react'

// ❌ Incorrect - Ne pas utiliser d'autres bibliothèques
...
- [ ] Icônes lucide-react utilisées (pas d'autres bibliothèques)
...
# Les composants HeroUI sont déjà installés via @heroui/* packages
# Importer directement depuis les packages
