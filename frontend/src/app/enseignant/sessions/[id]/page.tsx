...
import { SessionResultatsResume } from "@/features/enseignant/components/sessions/SessionResultatsResume";
...
export default function SessionDetailPage({ params }: SessionDetailPageProps) {
  ...
  const { data: qcmsData } = useSWR(["qcms-for-session-edit"], async () => {
  ...
  const handleMettreEnPause = async () => {
    if (!session) return;
    try {
      await sessionService.mettreEnPause(session.id);
      toast({
        title: "Session mise en pause",
        description: "La session a été mise en pause avec succès",
        variant: "success",
      });
      mutate();
    } catch (error: any) {
      ...
      toast({
        ...
        description:
          error.response?.data?.message ||
          "Erreur lors de la mise en pause de la session",
        variant: "error",
      });
    }
  };
  ...
  const handleReprendre = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "La session a été reprise avec succès",
        variant: "success",
      });
      mutate();
    } catch (error: any) {
      ...
      toast({
        ...
        description:
          error.response?.data?.message ||
          "Erreur lors de la reprise de la session",
        variant: "error",
      });
    }
  };
  ...
  return (
    <div className="container mx-auto max-w-full py-6 space-y-6">
      {/* Header */}
                    ...
                    <div className="p-4 space-y-4">
                      <div>
                        ...
                        <p className="text-sm text-default-600">
                          Cette session est actuellement en cours. La modifier
                          peut affecter les étudiants qui sont en train de
                          passer l'examen. Êtes-vous sûr de vouloir continuer ?
                        </p>
                      </div>
        ...
        <Card className="px-2">
          <CardHeader>
            <div className="flex items-center justify-between w-full">
              ...
              <Chip color={getStatusColor(session.status)} variant="flat">
            ...
            <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        variant="light"
                        onPress={() =>
                          router.push(`/enseignant/qcm/${session.qcmId}`)
                        }
                      >
                ...
                {session.classe && (
                  <div>
                    ...
                    <p className="font-medium">
                      {session.classe.nom} ({session.classe.code})
                    </p>
                  </div>
                )}
                <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-2 border-t border-default-200">
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {((session.notePassage / 20) * 100).toFixed(0)}%
                  </p>
                </div>
                <div>
                  <p className="text-sm text-default-500">Mélanger questions</p>
                  <Chip
                    color={session.melangeQuestions ? "success" : "default"}
                    size="sm"
                    variant="flat"
                  >
                ...
                <div>
                  ...
                  <Chip
                    color={session.melangeOptions ? "success" : "default"}
                    size="sm"
                    variant="flat"
                  >
      ...
      {!isEditing && session && (
        <SessionStatsSummary
          ...
          onOpenModal={onStatsModalOpen}
        />
      )}

      {/* Résultats de la session */}
      {!isEditing && session && session.status === "terminee" && (
        <SessionResultatsResume sessionId={session.id} />
      )}
          ...
          <CardBody>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Button
                ...
                variant="flat"
                onPress={() =>
                  router.push(`/enseignant/sessions/${session.id}/resultats`)
                }
              >
                Voir les résultats
              </Button>
