...
export function transformSessionToExamen(session: any): Examen {
  ...
  const nombreQuestions = session.nombreQuestions || session.nombre_questions || 0
  ...
  // Récupérer le niveau
  const niveau = session.niveau || session.classe?.niveau?.nom || 'Non spécifié'
  ...
  return {
    ...
    statut: (session.statut || mapStatusToFrontend(session.status || 'programmee')) as Examen['statut'],
 ...
 */
export function transformQuestionToQuestion(question: QuestionBackend): Question {
 ...
 */
export function transformResultatToResultat(resultat: ResultatBackend): Resultat {
  return {
    id: resultat.id,
 ...
 */
function transformReponsesDetail(reponsesDetail: Record<string, any>): Resultat['reponses'] {
