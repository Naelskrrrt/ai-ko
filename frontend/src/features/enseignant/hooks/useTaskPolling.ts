  ...
  const checkTaskStatus = useCallback(async () => {
    ...
    try {
      ...
      if (
        ...
        status.result?.progress !== undefined
      ) {
        ...
        if (status.result.estimated_remaining_seconds !== undefined) {
          setEstimatedRemainingSeconds(
            status.result.estimated_remaining_seconds,
          );
        } else if (
          estimatedDurationSeconds &&
          status.result.elapsed_seconds !== undefined
        ) {
          // Calculer le temps restant basé sur la progression
          const remaining =
            estimatedDurationSeconds - status.result.elapsed_seconds;
      ...
      console.error(
        "Erreur lors de la vérification du statut de la tâche:",
        error,
      );
      cleanup();
