...
export function QCMStatsSummary({ qcmId, onOpenModal }: QCMStatsSummaryProps) {
  const { data: stats, isLoading, error } = useSWR<QCMStatistics>(
  ...
  if (isLoading) {
    return (
      <Card className="border-none shadow-sm hover:shadow-md transition-shadow" isPressable onPress={onOpenModal}>
  ...
  if (error || !stats) {
    return (
      <Card className="border-none shadow-sm">
        <CardBody className="p-4">
          <p className="text-sm text-default-500">Aucune statistique disponible</p>
        </CardBody>
      </Card>
    )
  }
        ...
        <CardBody className="p-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-default-600">Statistiques</p>
              <p className="text-xs text-default-400 mt-1">Aucune soumission pour le moment</p>
            </div>
          </div>
        </CardBody>
      </Card>
      ...
      <CardBody className="p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 flex-1">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-default-600">Statistiques du QCM</p>
              <div className="flex items-center gap-4 mt-2">
                <div className="flex items-center gap-1.5">
                  <Users className="w-4 h-4 text-default-400" />
                  <span className="text-xs text-default-500">
                    {stats.nombre_soumissions} soumission{stats.nombre_soumissions > 1 ? 's' : ''}
                  </span>
                </div>
                <div className="flex items-center gap-1.5">
          ...
          <div className="text-xs text-default-400">Cliquer pour voir les détails</div>
        </div>
      </CardBody>
    </Card>
  )
