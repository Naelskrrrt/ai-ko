...
export const qcmService = {
   ...
   */
  async envoyerAuxEleves(id: string): Promise<{ qcm_id: string; nombre_etudiants: number; matiere: string }> {
   ...
   */
  async getQuestions(qcmId: string): Promise<{ questions: Question[]; total: number }> {
   ...
   */
  async createQuestion(qcmId: string, data: Partial<Question>): Promise<Question> {
   ...
   */
  async updateQuestion(qcmId: string, questionId: string, data: Partial<Question>): Promise<Question> {
    const response = await qcmApi.put(`/qcm/${qcmId}/questions/${questionId}`, data)
    return response.data
  },
   ...
   */
  async generateFromDocument(params: GenerateFromDocumentParams): Promise<TaskStatus> {
