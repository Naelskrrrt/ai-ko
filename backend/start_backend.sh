...
if [ ! -d "venv" ]; then
    ...
    echo "   Ou exécutez: ./install_and_create_venv.sh"
    exit 1
fi

# Vérifier si activate existe
if [ ! -f "venv/bin/activate" ]; then
    echo "⚠️  Le venv est incomplet (fichier activate manquant)"
    echo "💡 Recréez-le avec: ./install_and_create_venv.sh"
    exit 1
fi
...
flask db upgrade || echo "⚠️  Avertissement: Problème avec les migrations (peut être normal si la DB n'existe pas encore)"
